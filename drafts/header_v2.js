/**
 * Decorates the header block for AEM Edge Delivery
 * @param {Element} block - The header block element
 */
export default function decorate(block) {
  // Create the header structure based on the existing OU Health design
  const headerContent = `
    <!-- Utility Navigation -->
  ...
  // Clear the block and add the header content
  block.innerHTML = headerContent;

  // Search functionality
  const searchBtn = block.querySelector('.search-btn');
  const searchOverlay = block.querySelector('.search-overlay');
  const searchClose = block.querySelector('.search-close');
  ...
  const mobileMenuToggle = block.querySelector('.mobile-menu-toggle');
  const mobileMenu = block.querySelector('.mobile-menu');
  const mobileMenuClose = block.querySelector('.mobile-menu-close button');
